...
def test_if_a_certain_property_is_zero(halign, valign, prop_name):
    '''If one of [aspect_ratio, width, height] is zero, the size of the childshould be (0, 0)'''
    from kivy.uix.widget import Widget
    from kivyx.uix.aspectratio import KXAspectRatio, halign2attr, valign2attr
    ar = KXAspectRatio(pos=(10, 20, ), size=(30, 40), halign=halign, valign=valign)
    ...
    assert w.size == [0, 0]
    assert w.pos == [
        getattr(ar, halign2attr[halign]),
        getattr(ar, valign2attr[valign]),
    ]
...
def test_normal_situation(halign, valign, aspect_ratio, expected_size, expected_pos):
    from kivy.uix.widget import Widget
    from kivyx.uix.aspectratio import KXAspectRatio
    ar = KXAspectRatio(pos=(100, 200), size=(100, 100), halign=halign, valign=valign, aspect_ratio=aspect_ratio)
    w = Widget()
    ar.add_widget(w)
    ar.do_layout()
